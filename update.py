...
class Database:
    def __init__(self,master,*args,**kwargs):
         self.master=master
         self.heading=Label(master,text="Update Products",font=('arial 22 bold'),fg='black',bg='orange')
         ...
         self.name_l=Label(master,text="Enter Product Name",font=('arial 16 bold'))
         self.name_l.place(x=0,y=120)

         self.stock_l=Label(master,text="Enter Stocks",font=('arial 16 bold'))
         self.stock_l.place(x=0,y=170)

         self.cp_l = Label(master, text="Enter Cost Price ", font=('arial 16 bold'))
         self.cp_l.place(x=0, y=220)

         self.sp_l = Label(master, text="Enter Selling Price", font=('arial 16 bold'))
         self.sp_l.place(x=0, y=270)

         self.totalcp_l = Label(master, text="Enter Total Cost Price", font=('arial 16 bold'))
         self.totalcp_l.place(x=0, y=320)

         self.totalsp_l = Label(master, text="Enter Total Selling Price", font=('arial 16 bold'))
         self.totalsp_l.place(x=0, y=370)

         self.vendor_l = Label(master, text="Enter Vender Name", font=('arial 16 bold'))
         self.vendor_l.place(x=0, y=420)

         self.vendor_phone_l = Label(master, text="Enter Vendor Phone Number", font=('arial 16 bold'))
         ...
         self.btn_add=Button(master,text='Update Database',width=25,height=2,bg='orange',fg='black',command=self.update)
    ...
    def update(self,*args,**kwargs):
          ...
          query="UPDATE  inventory SET name=?,stock=?,cp=?,sp=?,totalcp=?,totalsp=?,vendor=?,vendor_phoneno=?  WHERE pid=?"
